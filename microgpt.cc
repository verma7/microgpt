// C++ implementation inspired by Andrej Karpathy's MicroGPT:
...
public:
  ...
  Value(float data, const vector<Ptr> &children = {},
        const vector<float> &local_grads = {})
      : data(data), grad(0), children_(children.begin(), children.end()),
  ...
  void backward() {
    ...
    for (auto &value : topo)
      for (int i = 0; i < value->children_.size(); i++)
        value->children_[i]->grad += value->local_grads_[i] * value->grad;
  }
...
vector<vector<Value::Ptr>> matrix(int nout, int nin,
                                  normal_distribution<float> &d, mt19937 &g) {
...
vector<Value::Ptr> linear(vector<Value::Ptr> &x,
                          vector<vector<Value::Ptr>> &w) {
...
vector<Value::Ptr>
gpt(int token_id, int pos_id, vector<vector<vector<Value::Ptr>>> *keys,
    vector<vector<vector<Value::Ptr>>> *values,
    unordered_map<string, vector<vector<Value::Ptr>>> *state_dict, int n_layer,
    int n_head, int head_dim) {
  vector<Value::Ptr> tok_emb =
      (*state_dict)["wte"][token_id]; // token embedding
  vector<Value::Ptr> pos_emb =
      (*state_dict)["wpe"][pos_id]; // position embedding
  vector<Value::Ptr> x;
  for (int i = 0; i < tok_emb.size(); i++)
    x.push_back(tok_emb[i] + pos_emb[i]); // joint token and position embedding
  x = rmsnorm(x); // note: not redundant due to backward pass via the residual
                  // connection
  ...
  for (int li = 0; li < n_layer; li++) {
    ...
    x = rmsnorm(x);
    vector<Value::Ptr> q =
        linear(x, (*state_dict)["layer{" + to_string(li) + "}.attn_wq"]);
    vector<Value::Ptr> k =
        linear(x, (*state_dict)["layer{" + to_string(li) + "}.attn_wk"]);
    vector<Value::Ptr> v =
        linear(x, (*state_dict)["layer{" + to_string(li) + "}.attn_wv"]);
    (*keys)[li].push_back(k);
...
int main() {
  ...
  int block_size = 16; // maximum context length of the attention window (note:
                       // the longest name is 15 characters)
  ...
  for (int step = 0; step < num_steps; step++) {
    ...
    for (int pos_id = 0; pos_id < n; pos_id++) {
      ...
      vector<Value::Ptr> logits = gpt(token_id, pos_id, &keys, &values,
