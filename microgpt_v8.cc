// V8 = V7 + techniques from the 2024-25 nanoGPT-speedrun literature:
//   --muon       Muon optimizer for hidden matrices: Nesterov momentum
//                orthogonalized by 5 Newton-Schulz iterations
//                (a,b,c = 3.4445, -4.7750, 2.0315); Adam keeps embeddings,
//                head, and all vector/scalar params. --muonlr sets its LR.
//   --mlp relu2  ReLU^2 activation (h = relu(z)^2)
//   --valemb     per-layer value embeddings: a zero-init [vocab,E] table
//                added to V after projection (token identity at depth)
//   --softcap C  logit soft-capping: logits = C*tanh(z/C)
//   LR schedule floors at 0.1x peak; Muon momentum warms 0.85->0.95.
//
...
// Newton-Schulz orthogonalization (5 iterations, quintic coefficients from
// Keller Jordan's Muon). X is [m,n] row-major with m <= n; in-place.
static void newton_schulz5(float *X, int m, int n, float *A, float *A2,
                           float *BX) {
  const float ca = 3.4445f, cb = -4.7750f, cc = 2.0315f;
  double fro = 0;
  for (size_t i = 0; i < (size_t)m * n; i++)
    fro += (double)X[i] * X[i];
  float inv = 1.0f / ((float)sqrt(fro) + 1e-7f);
  for (size_t i = 0; i < (size_t)m * n; i++)
    X[i] *= inv;
  for (int it = 0; it < 5; it++) {
    gemm(false, true, m, m, n, 1, X, n, X, n, 0, A, m);   // A = X X^T
    gemm(false, false, m, m, m, 1, A, m, A, m, 0, A2, m); // A2 = A A
    for (size_t i = 0; i < (size_t)m * m; i++)
      A[i] = cb * A[i] + cc * A2[i]; // B = b A + c A^2
    gemm(false, false, m, n, m, 1, A, m, X, n, 0, BX, n); // BX = B X
    for (size_t i = 0; i < (size_t)m * n; i++)
      X[i] = ca * X[i] + BX[i];
  }
}
...
struct Model {
  ...
  vector<int> o_ve;                              // v8: value embeddings [V,E]
  int wd_end; // params below this index get weight decay (the matrices)
  struct MuonMat { int off, rows, cols; };
  vector<MuonMat> muon_mats; // hidden matrices updated by Muon
  int NP;
  vector<float> P, G, am, av;
  vector<uint8_t> is_muon; // per-param: updated by Muon, not Adam
  bool swiglu, gelu, relu2;
  float inv_sqrt_d;
  ...
  void init(mt19937 &rng) {
    ...
    for (int l = 0; l < L; l++) {
      o_wq[l] = off; muon_mats.push_back({off, E, E}); off += E * E;
      o_wk[l] = off; muon_mats.push_back({off, E, E}); off += E * E;
      o_wv[l] = off; muon_mats.push_back({off, E, E}); off += E * E;
      o_wo[l] = off; muon_mats.push_back({off, E, E}); off += E * E;
      o_fc1[l] = off; muon_mats.push_back({off, M, E}); off += M * E;
      o_fc2[l] = off; muon_mats.push_back({off, E, M}); off += E * M;
      if (swiglu) { o_fc3[l] = off; muon_mats.push_back({off, M, E}); off += M * E; }
    }
    ...
    o_ve.assign(L, -1);
    if (cfg.valemb) // zero-init token->V tables, Adam-updated, no decay
      for (int l = 0; l < L; l++) { o_ve[l] = off; off += V * E; }
    int H = cfg.heads;
    for (int l = 0; l < L; l++) {
    ...
    for (int i = 0; i < NP; i++)
      P[i] = d(rng);
    if (cfg.valemb) // value embeddings start as a no-op
      for (int l = 0; l < L; l++)
        fill(P.begin() + o_ve[l], P.begin() + o_ve[l] + V * E, 0.0f);
    is_muon.assign(NP, 0);
    if (cfg.muon)
      for (auto &mmat : muon_mats)
        fill(is_muon.begin() + mmat.off,
             is_muon.begin() + mmat.off + (size_t)mmat.rows * mmat.cols, 1);
    if (cfg.residscale) {
  ...
  float run_window(const uint8_t *tokens, bool accumulate_grads) {
    ...
    for (int l = 0; l < L; l++) {
      ...
      gemm(false, true, T_, E_, E_, 1, xs, E_, Pp + mm.o_wv[l], E_, 0, vproj,
           E_);
      if (mm.cfg.valemb) // token-conditioned additive value embedding
        for (int t = 0; t < T_; t++) {
          const float *ve = Pp + mm.o_ve[l] + (size_t)tokens[t] * E_;
          float *vt = vproj + (size_t)t * E_;
          for (int i = 0; i < E_; i++)
            vt[i] += ve[i];
        }
      if (mm.cfg.valres) {
    ...
    gemm(false, true, T_, Vc, E_, 1, head_in, E_, Pp + mm.o_lm, E_, 0,
         Probs.data(), Vc);
    if (mm.cfg.softcap > 0) { // logits = C*tanh(z/C); tanh saved for bwd
      float C = mm.cfg.softcap;
      for (size_t i = 0; i < (size_t)T_ * Vc; i++) {
        float th = tanhf(Probs[i] / C);
        CapT[i] = th;
        Probs[i] = C * th;
      }
    }
    ...
    if (mm.cfg.softcap > 0) // through z = C*tanh(raw/C): dz_raw = dz*(1-th^2)
      for (size_t i = 0; i < (size_t)T_ * Vc; i++)
        Probs[i] *= 1.0f - CapT[i] * CapT[i];
    ...
    for (int l = L - 1; l >= 0; l--) {
      ...
      if (mm.cfg.valemb) // dV rows flow into this layer's value embedding
        for (int t = 0; t < T_; t++) {
          float *gve = &Glocal[mm.o_ve[l] + (size_t)tokens[t] * E_];
          const float *dvt = dV.data() + (size_t)t * E_;
          for (int i = 0; i < E_; i++)
            gve[i] += dvt[i];
        }
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    else if (a == "--softcap") cfg.softcap = stof(next());
  ...
  cout << "config: layers=" << cfg.layers << " embd=" << cfg.embd
       ...
       << " softmax1=" << cfg.softmax1 << " muon=" << cfg.muon
       << " valemb=" << cfg.valemb << " softcap=" << cfg.softcap
  ...
  for (int step = 0; step < cfg.steps; step++) {
    ...
    if (step < cfg.warmup)
      ...
      lr_t = cfg.lr * (0.1f + 0.9f * 0.5f * (1 + cosf(prog * (float)M_PI)));
    }
    ...
    float bc2 = 1 - powf(beta2, step + 1);
    {
      ...
      // phase 2: AdamW (skips Muon-owned params)
      vector<thread> at;
      for (int ci = 0; ci < nch; ci++)
        at.emplace_back([&, ci]() {
          long lo = ci * chunk, hi = min<long>(model.NP, lo + chunk);
          for (long i = lo; i < hi; i++) {
            if (cfg.muon && model.is_muon[i])
              continue;
      ...
      for (auto &t : at)
        t.join();
      // phase 3: Muon for hidden matrices — Nesterov momentum (warmed
      // 0.85 -> 0.95), Newton-Schulz orthogonalization, spectral-scaled step
      if (cfg.muon) {
        float mu = 0.85f + 0.10f * min(1.0f, (float)step / 300.0f);
        float mlr = cfg.muonlr * (lr_t / cfg.lr); // same schedule shape
        atomic<int> next_m(0);
        vector<thread> mt;
        for (int ti = 0; ti < cfg.threads; ti++)
          mt.emplace_back([&]() {
            vector<float> X, A, A2, BX;
            int mi;
            while ((mi = next_m.fetch_add(1)) < (int)model.muon_mats.size()) {
              auto &mm2 = model.muon_mats[mi];
              int r = mm2.rows, c = mm2.cols;
              int a = min(r, c), b = max(r, c);
              bool tr = r > c; // NS runs on [a,b] with a<=b
              X.resize((size_t)a * b); A.resize((size_t)a * a);
              A2.resize((size_t)a * a); BX.resize((size_t)a * b);
              float *M = &model.am[mm2.off]; // momentum buffer
              float *g0 = &model.G[mm2.off];
              float *p0 = &model.P[mm2.off];
              for (size_t i = 0; i < (size_t)r * c; i++)
                M[i] = mu * M[i] + g0[i] * scale;
              // Nesterov update direction into X (transposed if needed)
              for (int i = 0; i < r; i++)
                for (int j = 0; j < c; j++) {
                  float u = g0[(size_t)i * c + j] * scale +
                            mu * M[(size_t)i * c + j];
                  if (tr)
                    X[(size_t)j * r + i] = u;
                  else
              ...
              newton_schulz5(X.data(), a, b, A.data(), A2.data(), BX.data());
              float sc = mlr * sqrtf(max(1.0f, (float)r / (float)c));
              for (int i = 0; i < r; i++)
                for (int j = 0; j < c; j++) {
                  float o = tr ? X[(size_t)j * r + i] : X[(size_t)i * c + j];
                  p0[(size_t)i * c + j] -= sc * o;
                }
              if (cfg.wd > 0)
                for (size_t i = 0; i < (size_t)r * c; i++)
                  p0[i] -= mlr * cfg.wd * p0[i];
            }
          });
        for (auto &t : mt)
          t.join();
